...
$(document).ready(function(){
       ...
       $(document).on('change keyup blur', '#type', function() {
           var type = $(this).val();
           if(type === '1'){ //inter company
              $('#warehouselist').show();
              $('#supplierlist').hide();
           }
           else if(type === '0'){ //out of company
              $('#warehouselist').hide();
              $('#supplierlist').show();
           }
        });

      var recieptFrom = $('#type').val();
      var fromWhere = null;

      if(recieptFrom=='1')
      {
       fromWhere = $("#pickup_godown_id").val();
      }
      else if (recieptFrom=='0') {
        fromWhere = $("#supplier").val();
      }

    if(recieptFrom==''){ alert("Please selet type"); }
    if(fromWhere===null && recieptFrom === '1'){ alert("please selet source godown"); }
    if(fromWhere===null && recieptFrom === '0'){ alert("please selet supplier"); }
